...
public class GUICreateUpdateFornecedor extends JDialog {
	...
	public GUICreateUpdateFornecedor(final int modo, ILaboratorio lab) {
		setResizable(false);
		...
		if (modo != -1) {
			...
			tfNome.setText(f.get(modo).getNome());
			tfCNPJ.setText(f.get(modo).getCnpj());
			tfTelefone.setText(String.valueOf(f.get(modo).getTel()));
			tfLogra.setText(String.valueOf(f.get(modo).getLogra()));
			tfNum.setText(String.valueOf(f.get(modo).getNum()));
			tfComp.setText(f.get(modo).getComp());
			tfBairro.setText(f.get(modo).getBairro());
			tfCidade.setText(f.get(modo).getCidade());
			cod = f.get(modo).getCod();
			if (f.get(modo).getComp()!=null) {
	...
	protected void alterar() {
		...
		if (verificarCampos()) {
			...
			try {
				...
				if(!(lab.alterFornecedor(cod, nome, telefone, logra, num, bairro, cidade, cnpj, comp)))
					finalTest = false;
				if (finalTest) {
					JOptionPane.showMessageDialog(this, "Fornecedor alterado com sucesso", "Alterar de forneceodr", JOptionPane.INFORMATION_MESSAGE);
					dispose();
				}
				else {
	...
	protected void novo() {
		...
		if (verificarCampos()) {
			...
			try {
				long telefone = Long.parseLong(tel);
				int num = Integer.parseInt(numero);
				if(!(lab.newFornecedor(nome, telefone, logra, num, bairro, cidade, cnpj, comp)))
					finalTest = false;
				if (finalTest) {
