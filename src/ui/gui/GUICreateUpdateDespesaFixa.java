...
public class GUICreateUpdateDespesaFixa extends JDialog {
	...
	public GUICreateUpdateDespesaFixa(ILaboratorio l) {
		lab = l;
		setTitle("Registrar despesa fixa");
		setIconImage(Toolkit.getDefaultToolkit().getImage(getClass().getResource("/res/icon.png")));
		setModal(true);
		setResizable(false);
		setBounds(100, 100, 448, 191);
		...
		lblObrigatrio.setBounds(10, 99, 112, 13);
		...
		if (verificaCampos(nome, valor)) {
			...
			try {
				if (valor.contains(","))
					...
					if (lab.newDespesaFixa(nome, valorMedio)) {
						JOptionPane.showMessageDialog(GUICreateUpdateDespesaFixa.this, "Despesa fixa registrada com sucesso",
								"Registrar despesa fixa", JOptionPane.INFORMATION_MESSAGE);
						dispose();
					} else {
						JOptionPane.showMessageDialog(GUICreateUpdateDespesaFixa.this,
